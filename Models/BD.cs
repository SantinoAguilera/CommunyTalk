...
public static class BD
{
    ...
    public static int BuscarIDInteres(string Nombre)
    {
        using (SqlConnection db = new SqlConnection(_connectionString))
        {
            string sql = "SELECT IdInteres FROM Intereses WHERE Nombre = @pNombre";
            return db.QueryFirstOrDefault<int>(sql, new { pNombre = Nombre });
        }
    }
